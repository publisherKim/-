...
미들웨어가 말이 좀 그럴듯해서 어려워보이긴 하지만, 쉽게 얘기하면 각 라우트에서 해야할 공통의 작업들을 미리 정의해서 처리하는 것을 말한다. 예를들어 post의 변수들을 파싱한다든가 인증처리를 먼저 한다든가하는 것들이다. 물론 프로그램의 모든 것이 그러하든 미들웨어 필요없어! 이러면서 각 라우트별로 다 작성을 해도 되긴 된다.
